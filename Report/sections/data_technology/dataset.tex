...
ogni settimana, nonchè i risultati del test per il virus del Nilo occidentali. 
È stato reperito dal sito 
\href{https://data.cityofchicago.org/Health-Human-Services/West-Nile-Virus-WNV-Mosquito-Test-Results/jqe8-8r6s/data}{Chicago
 Data Portal}.
\\
...
dalla 
\href{https://www.ncdc.noaa.gov/data-access/land-based-station-data/land-based-datasets/quality-controlled-local-climatological-data-qclcd}{National
Oceanic and Atmospheric Administration}. I dati grezzi sono composti da un file 
CSV per ciascun mese, al cui interno sono presenti le rilevazioni di tutte le 
stazioni meteorologiche degli Stati Uniti per il periodo 2007--2017. 
% FIXME: ci sono anche i file delle stazioni.

Come prima cosa abbiamo scritto una serie di script Bash con i quali estrarre 
esclusivamente le rilevazioni delle stazioni meteorologiche nei pressi 
della città di Chicago e aggregarle in un unico file contenente l'intero arco 
temporale di interesse. Attraverso un ulteriore script Bash abbiamo analizzato 
le colonne presenti nei file CSV, per accertarci che la loro struttura fosse 
rimasta costante negli anni.
...
l'anno 2018 dak dataset WNV Mosquito.
...
\begin{longtable}{lll}
	...
	&& GRAVID\\
	&& SENTINEL\\\hline
	{test\_date}	&{data del test}   &   yyyy/mm/dd hh:mm:ss \\\hline
...
\begin{longtable}{lll}
	...
	\hline	
	{year\_month\_day} & {data della rilevazione}	 & dd mmmm,yyyy \\ \hline	
	...
	& & FG = nebbia \\
	& & FU = fumo \\
	& lista di codici del tipo & GR = grandine \\
	& di tempo giornaliero & GS = piccola grandine\\
	& & HZ = foschia \\
	& & PL = pioggia gelata\\
	& & RA = pioggia \\
	& & SN = neve \\
	...
	& & s = valore sospetto			 \\ 
	\hline
	\multirow{2}{*}{precip\_total}		& quantità di precipitazione 	 & \multirow{2}{*}{decimale}			\\ 
	& giornaliera (inch) &\\\hline	
	\multirow{2}{*}{precip\_total\_flag}		& \multirow{2}{*}{qualità precip\_total} &  blank = valore non sospetto \\
	& & s = valore sospetto			 \\ 
	\hline	
	\multirow{2}{*}{stn\_pressure}		& pressione giornaliera	 & 	\multirow{2}{*}{decimale}		\\ 
	&   (pollici di mercurio) &\\
	\hline	
	\multirow{2}{*}{stn\_pressure\_flag}		& \multirow{2}{*}{qualità stn\_pressure} &  blank = valore non sospetto \\
	& & s = valore sospetto			 \\ 
	\hline	
	\multirow{3}{*}{sea\_level}			& pressione giornaliera 	 & 	\multirow{3}{*}{decimale}		\\ 
...
\begin{longtable}{lll}
	...
	\endfoot
	\endlastfoot
	wban	&   id della stazione & numerico   			\\ \hline	
	\multirow{3}{*}{wmo}	& 	codice della stazione   & \multirow{3}{*}{numerico}	\\ 
	& secondo l'organizzazione  & \\
	& meteorologica mondiale & \\ \hline	
	\multirow{2}{*}{callsign}	& 	codice di denominazione  & \multirow{2}{*}{categorico}	    \\
	& della stazione & \\ \hline
	climate\_division\_code				&	codice ... 	& numerico	\\ \hline	
	climate\_division\_state\_code		& 	 			& numerico	\\ \hline
	climate\_division\_station\_code	& 	 			& numerico	\\ \hline	
