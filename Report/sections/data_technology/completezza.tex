...
Il conteggio dei valori mancanti è stato effettuato tramite il software Power BI di Microsoft, in particolare ricorrendo allo strumento \textbf{Power Query}.
...
\begin{figure}[H]
	...
	\begin{tabular}{lcccc}
		...
		\bottomrule
	\end{tabular}
	\captionof{table}{Analisi di completezza sul dataset \textsc{WNV Mosquito}}
...
In tabella \ref{tab:completezza weather} sono presentate le analisi di completezza relative a ciascun attributo e la percentuale della completezza di tupla e di schema.
	...
	\end{longtable}
	\captionof{table}{Analisi di completezza sul dataset \textsc{Weather}}
	\label{tab:completezza weather}

\subsubsection*{Stations}
\addcontentsline{toc}{subsubsection}{Stations}
Il dataset \textsc{Stations} presenta una completezza del 82,67\%. In 
percentuale esso risulta essere il meno completo tra i dataset presentati, in 
valori assoluti è però molto più piccolo degli altri, dato che contiene solo 5 
tuple. Inoltre, dei quattro attributi su 15 che presentano dei valori mancanti, 
possiamo trascurare \texttt{wmo}, \texttt{climate\_division\_code} e 
\texttt{climate\_division\_station\_code}, trattandosi di codici identificativi 
delle stazioni meteorologiche che non sono importanti per i nostri scopi. \\

In tabella \ref{tab:completezza stations} sono presentate le analisi di 
completezza relative a ciascun attributo, alle tuple e all'intero schema.
...
\begin{figure}[H]
	...
	\begin{tabular}{lcccc}
		...
		\bottomrule
	\end{tabular}
	\captionof{table}{Analisi di completezza sul dataset \textsc{Stations}}
	\label{tab:completezza stations}
\end{figure}

\subsection{Acquisizione di nuovi dati}
...
Nel dataset \textsc{WNV Mosquito} è stato possibile popolare i valori 
\texttt{latitude} e \texttt{longitude} mancanti, utilizzando il servizio di 
\textbf{Geocoding}\footnote{\url{https://developers.google.com/maps/documentation/geocoding/start}}
